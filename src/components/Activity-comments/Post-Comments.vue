<template>
  <div class="comments">
    <div class="comments-top">
      ...
      <div
        class="comments-btn"
        @click="ActivityComment({ comment: discuss, type: 0 })"
      >
        评论
      </div>
    </div>
        ...
        <li v-for="(item, index) in comments" :key="index">
          ...
          <div class="info">
            ...
            <div class="comments-options">
              ...
              <!-- 点赞 -->
              <span class="fa" @click="handelLiked(item, 1)">
                <i
                  class="iconfont"
            ...
            <div class="comments-reply" v-if="item.replyList.length > 0">
              <ul>
                <li v-for="(comment, i) in item.replyList" :key="i">
                  <img :src="comment.avatar" alt="" />
                  <div class="info">
                    <div class="critics-name">{{ comment.nickname }}</div>
                    <p>回复 {{ comment.replyName }}:{{ comment.content }}</p>
                    <div class="comments-options">
                      <!-- 时间 -->
                      <span>{{ comment.recentlyVisited }}</span>
                      <!-- 点赞 -->
                      <span class="fa" @click="handelLiked(comment, 2)">
                        <i
                          class="iconfont"
                          :style="{
                            color:
                      ...
                      <div
                        ...
                        @click="
                          ActivityComment({
                            ...
                            type: 1,
                            commentId: item.id,
                            superiorId: comment.id,
                            replyName: comment.nickname,
                          })
                        "
                      >
                        评论
              ...
              <div
                ...
                v-if="haveMore !== index && item.theRemainingAmount > 0"
              >
...
export default {
  ...
  methods: {
    ...
    // 获取海报评论列表
    getTopicComment() {
      this.$api.getActivityComment(this.pageForm).then((res) => {
          ...
          if (res.data.code == 200) {
            this.comments.forEach((element) => {
              if (element.id == item.id) {
                element.replyList = res.data.data;
              }
            });
            this.haveMore = index;
          }
    ...
    ActivityComment(item) {
      if (item.comment) {
        ...
        let data = {
          ...
          type: item.type,
          workspaceId: this.$store.getters.workspaceId,
          commentsId: item.commentId ? item.commentId : "",
          superiorId: item.superiorId ? item.superiorId : "",
          replyName: item.replyName,
        };
        if (this.flag) {
          this.flag = false;
          this.$api.ActivityComment(data).then((res) => {
            if (res.data.code == 200) {
              this.$notify({
                title: "成功",
                message: "评论成功",
    ...
    handelLiked(item, type) {
      ...
      this.$api.ActivityThumbUp(commentLikedParameter).then((res) => {
        if (res.data.code == 200) {
          this.getTopicComment();
        }
      });
